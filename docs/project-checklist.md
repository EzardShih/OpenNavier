...
junior simulation engineer under review. Diagnostics, schema-backed case-build
specs, runners, and
...
Gemini CLIs. The model response must still flow through typed specs,
`CaseBuildSpec` artifacts, plans, tool calls, diffs, and approval points instead
of directly mutating CAD, mesh, or OpenFOAM files.
...
| 3 | Deterministic junior-engineer workflow | Planner, schema-backed case-build specs, validators, runner, diagnostics, reports, CLI/MCP contracts. | A no-LLM workflow can inspect a request, create a plan, prepare a case, run checks, and summarize evidence. |
...
| P0 | Simulation plan artifact | Convert a validated `SimulationSpec` plus validated `CaseBuildSpec` into an explicit plan with assumptions, required tools, commands, expected artifacts, pre-flight checks, and approval checkpoints. | Plan tests cover accepted cavity shape, build-spec references, approval points, required executable fields, and stable JSON output. | The plan is the bridge between engineering intent, build-spec validation, and deterministic tools. |
...
| P0 | Schema-driven planner baseline | Map supported specs and validated `CaseBuildSpec` artifacts to validation steps, run commands, expected logs, and report outputs without using a model. | Tests cover cavity build-spec planning, build-spec schema validation, rejection of raw dictionary text, not-yet-executable physics, and deterministic ordering. | Build the planner and build-spec schema before `ask`; the model should propose typed build data, then request a plan. |
...
| P1 | Clarifying questions | Detect missing or ambiguous simulation inputs and return concrete questions instead of guessing. | Tests cover missing geometry, fluid properties, boundary conditions, objective, units, and not-yet-executable case classes. | This belongs before natural-language execution because the agent should ask before creating a misleading case. |
| P1 | Pipe or duct build-spec coverage | Add schema-backed pipe-flow or duct-pressure-drop build behavior through deterministic writer operations. | Build-spec tests compare generated files, overwrite guards, validation results, and committed examples where used. | These expand executable coverage after the session, planner, and run contracts are stable. |
| P1 | Solver compatibility checks | Validate that selected solver family, dictionaries, fields, and build-spec assumptions agree for supported case-build specs. | Unit tests cover valid cavity/duct cases and known incompatible solver/dictionary combinations. | Keep this deterministic and separate from LLM planning. |
...
| P2 | LLM intent interface | Add `opennavier ask` or equivalent MCP flow that turns natural language into a draft spec, clarifying questions, `CaseBuildSpec`, or a plan. | Tests use mocked model responses and verify schema validation, build-spec handoff, rejection handling, provenance, and no direct dictionary writes. | LLM output must enter through `SimulationSpec`, question, `CaseBuildSpec`, or plan artifacts. |
...
| Examples | Reproducible `examples/cavity` case | Done | Done | `examples/cavity`; `tests/test_examples.py::test_committed_cavity_example_matches_template_and_validates_read_only` | Keep committed examples as non-executable prompt guidance and validated reference cases, free of runtime artifacts. |
| CLI | `opennavier init cavity <case_path>` case creation | Done | Done | `tests/test_init.py` | Treat existing init behavior as legacy starter-case support while the schema-backed case-build path is added. |
...
| OpenFOAM validation | Solver compatibility validation | Not Started | Not Started | No solver-family-to-dictionary compatibility validator exists yet. | Add tests for supported solver/build-spec pairs and known incompatible dictionaries. |
| OpenFOAM case build | Deterministic lid-driven cavity case generation | Done | Done | `packages/openfoam/src/opennavier/openfoam/init_case.py`; `tests/test_init.py` | Treat current cavity generation as the first schema-backed build capability and keep generated output validated. |
| OpenFOAM case build | Pipe-flow or duct-pressure-drop build-spec generation | Not Started | Not Started | `docs/plan.md` identifies these as next supported cases. | Add schema-backed build behavior and committed example coverage only after generated dictionaries and overwrite guards are specified in tests. |
...
| MCP | MCP cavity case initialization and structure validation tools | Done | Done | `packages/mcp/src/opennavier/mcp/case_init_tools.py`; `packages/mcp/src/opennavier/mcp/case_validation_tools.py`; `tests/test_mcp_case_init_cavity.py`; `tests/test_mcp_case_validate_structure.py` | Legacy direct cavity initialization remains tested, but server registration should prefer generic case-build tools. |
| MCP | MCP case-build validation and generation tools | Done | Done | `packages/mcp/src/opennavier/mcp/case_build_tools.py`; `tests/test_mcp_case_build_tools.py` | Path-safe `case_build_validate`, `case_build_dry_run`, and `case_build_write` wrap core/OpenFOAM APIs. |
...
| AI planning | Schema-driven case-build planner | Done | Done | `packages/core/src/opennavier_core/case_build_spec.py`; `packages/core/src/opennavier_core/planner.py`; `tests/test_case_build_spec.py`; `tests/test_planner.py` | Planner maps validated `SimulationSpec` plus validated `CaseBuildSpec` artifacts to commands and typed capability results. |
...
| AI planning | LLM-generated case-build specs | Not Started | Not Started | A future model may draft typed case-build data from prompt examples, but no schema/MCP validation loop exists yet. | Accept model output only as schema-validated `CaseBuildSpec` artifacts; reject raw OpenFOAM dictionary text and direct file mutation. |
