...
| `$env:UV_CACHE_DIR='.tmp\uv-cache'; uv run pytest` | Done | 132 tests passed; pytest warned that `.tmp\pytest-cache` could not be written. |
...
| `uv run opennavier --help` | Not rechecked | CLI help was not part of the MCP validation pass. |
| `$env:UV_CACHE_DIR='.tmp\uv-cache'; uv run opennavier --help` | Not rechecked | CLI help was not part of the MCP validation pass. |
...
| --- | --- | --- | --- | --- | --- |
| Repository | Python workspace with CLI, core, OpenFOAM, and MCP packages | Done | Done | `pyproject.toml`; `$env:UV_CACHE_DIR='.tmp\uv-cache'; uv run pytest` | Keep package boundaries stable as features are added. |
